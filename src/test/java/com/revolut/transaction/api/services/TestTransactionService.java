package com.revolut.transaction.api.services;
 ...
 * src/test/resources/transaction-api.sql
 */
public class TestTransactionService extends TestService {
    // test transaction related operations in the account

    /*
     * TC B1 Positive Category = AccountService Scenario: test deposit money to
     * given account number return 200 OK
     */
    @Test
    ...
    /*
     * TC B2 Positive Category = AccountService Scenario: test withdraw money from
     * account given account number, account has sufficient fund return 200 OK
     */
    @Test
    ...
    /*
     * TC B3 Negative Category = AccountService Scenario: test withdraw money from
     * account given account number, no sufficient fund in account return 500
     * INTERNAL SERVER ERROR
     */
    @Test
    public void testWithDrawNonSufficientFund() throws IOException, URISyntaxException {
        URI uri = builder.setPath("/account/2/withdraw/1000.23").build();
        HttpPut request = new HttpPut(uri);
        request.setHeader("Content-type", "application/json");
        HttpResponse response = client.execute(request);
        int statusCode = response.getStatusLine().getStatusCode();
    ...
    /*
     * TC B4 Positive Category = AccountService Scenario: test transaction from one
     * account to another with source account has sufficient fund return 200 OK
     */
    @Test
    public void testTransactionEnoughFund() throws IOException, URISyntaxException {
        ...
        TransactionDetails transaction = new TransactionDetails("EUR", amount, 3L, 4L);
    ...
    /*
     * TC B5 Negative Category = AccountService Scenario: test transaction from one
     * account to another with source account has no sufficient fund return 500
     * INTERNAL SERVER ERROR
     */
    @Test
    ...
    /*
     * TC B6 Negative Category = TransactionService Scenario: test transaction from
     * one account to another with source/destination account with different
     * currency code return 500 INTERNAL SERVER ERROR
     */
    @Test
    public void testTransactionDifferentCcy() throws IOException, URISyntaxException {
        ...
        TransactionDetails transaction = new TransactionDetails("USD", amount, 3L, 4L);
